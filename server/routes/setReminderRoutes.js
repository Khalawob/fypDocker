...
router.get("/:setId/reminder", requireAuth, (req, res) => {
  db.query(
    `SELECT reminder_id, reminder_enabled, interval_hours, next_review_at, last_sent_at
 ...
 * body: { reminder_enabled, interval_hours }
 */
router.post("/:setId/reminder", requireAuth, (req, res) => {
  const { reminder_enabled, interval_hours } = req.body;
  ...
  if (typeof reminder_enabled !== "boolean") {
    return res
      .status(400)
      .json({ message: "reminder_enabled must be true or false" });
  }

  const hours = Number(interval_hours || 24);

  if (reminder_enabled && (!hours || hours < 1)) {
    return res
      .status(400)
      .json({ message: "interval_hours must be at least 1" });
  }
    ...
    (setErr, setResults) => {
      ...
      db.query(
        `INSERT INTO set_review_reminder
          (user_id, set_id, reminder_enabled, interval_hours, next_review_at)
         VALUES (?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL ? HOUR))
         ON DUPLICATE KEY UPDATE
          reminder_enabled = VALUES(reminder_enabled),
          interval_hours = VALUES(interval_hours),
          next_review_at = CASE
            WHEN VALUES(reminder_enabled) = 1
              THEN DATE_ADD(NOW(), INTERVAL VALUES(interval_hours) HOUR)
            ELSE next_review_at
          END`,
        [req.user.userId, setId, reminder_enabled, hours, hours],
        (err) => {
          if (err) return res.status(500).json({ message: err.message });
